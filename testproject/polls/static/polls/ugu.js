! function(t, e, n, o, r) {
    ...
    function c() {
        K.setItem("avatar", JSON.stringify(loginAvatarData))
    }
    ...
    function l() {
        if (at)
            if (it && it.drawingID == it.myID) {
                if (ht.drawCommands.length > 0) {
                    if (ht.drawCommands.length > 8) {
                        var t = ht.drawCommands.splice(7);
                        at.emit("drawCommands", ht.drawCommands), ht.drawCommands = t
                    } else at.emit("drawCommands", ht.drawCommands), ht.drawCommands = []
                }
            } else ht.drawCommands.length > 0 && (ht.drawCommands = [])
    }
    ...
    function m() {
        at && at.emit("votekickCurrent"), y()
    }

    function g() {
        ut.goto("load");
        var t = null === K.getItem("firstLogin");
        t && K.setItem("firstLogin", "");
        var e = !1;
        if (!t) {
            var n = K.getItem("lastAd"),
                o = new Date;
            if (null === n || n === r || "undefined" === n) e = !0;
            else {
                var s = new Date(Date.parse(n));
                Math.abs(s - o) / 1e3 / 60 >= 2.5 && (e = !0)
            }
            e && K.setItem("lastAd", o.toString())
        }
        if (e) try {
    ...
    function w() {
        ct.context && ct.context.resume();
        var t = x(),
            e = io(location.origin + ":4999");
        e.emit("login", t), e.on("result", function(o) {
            o.code ? (n("#error-login").hide(), C(o.host, t)) : (n("#error-login").text(o.msg).show(), ut.goto("login")), e.close()
        })
    }
    ...
    function C(t, e) {
        ct.playSound("playerJoin"), at = io(t, {
            query: {
                token: yt
            }
        }), at.on("connect", function() {
            at.on("disconnect", function() {
                at.close(), at = null, it = null, ut.goto("login"), n("#modalDisconnect").modal()
            }), at.emit("userData", e), at.on("kicked", function() {
                n("#modalKicked").modal()
            }), at.on("drawCommands", function(t) {
                for (var e = 0; e < t.length; e++) I(t[e])
            }), at.on("canvasClear", function() {
                ht.clear()
            }), at.on("lobbyKicked", function(t) {
                var e = it.players.get(t);
                null != e && (it.chatAddMsg(null, e.name + " was kicked!", G), e.kicked = !0)
            }), at.on("chat", function(t) {
                var e = it.players.get(t.id),
                    n = it.players.get(it.myID),
                    o = e.id == it.drawingID || e.guessedWord;
                n.id != it.drawingID && !n.guessedWord && o || it.chatAddMsg(e, v(t.message), o ? "#7dad3f" : "#000")
            }), at.on("lobbySpam", function() {
                it.chatAddMsg(null, "Spam detected! You're sending too many messages.", G)
            }), at.on("restartNotification", function(t) {
                it && it.chatAddMsg(null, "Server will restart in about " + t + " seconds...", N)
            }), at.on("lobbyVotekickCurrent", function(t) {
                if (it) {
                    var e = it.players.get(t[0]),
                        n = it.players.get(t[1]);
                    it.chatAddMsg(null, "'" + e.name + "' is voting to kick '" + n.name + "' (" + t[2] + "/" + t[3] + ")", N)
                }
            }), at.on("lobbyConnected", function(t) {
                if (ht.clear(), E(""), it = new et(t), it.inGame) {
                    for (var e = 0; e < t.drawCommands.length; e++) I(t.drawCommands[e]);
                    ut.goto("game"), it.drawingID >= 0 && D(it.time)
                } else ut.goto("lobby")
            }), at.on("lobbyLobby", function() {
                ut.goto("lobby")
            }), at.on("lobbyState", function(t) {
                switch (parseInt(t[0])) {
                    case $:
                        d(), D(it.time);
                        break;
            ...
            }), at.on("lobbyDisconnected", function() {
                at.emit("lobbyLeave"), it = null, ut.goto("login")
            }), at.on("lobbyGameStart", function(t) {
                it.reset(), E(""), ut.goto("game")
            }), at.on("lobbyGameEnd", function() {
                var t = [];
                it.players.forEach(function(e, n, o) {
                    t.push(e)
                }), L({
                    mode: "endgame",
                    text: "Result",
            ...
            }), at.on("lobbyLanguage", function(t) {
                it.setLanguage(t)
            }), at.on("lobbyRounds", function(t) {
                it.setRounds(t)
            }), at.on("lobbyDrawTime", function(t) {
                it.setDrawTime(t)
            }), at.on("lobbyCustomWordsExclusive", function(t) {
                it.setCustomWordsExclusive(t)
            }), at.on("lobbyPlayerConnected", function(t) {
                it.addPlayer(t), it.chatAddMsg(null, t.name + " joined.", j), ct.playSound("playerJoin")
            }), at.on("lobbyPlayerDisconnected", function(t) {
                var e = it.players.get(t);
                it.removePlayer(e), e.kicked || it.chatAddMsg(null, e.name + " left.", G), ct.playSound("playerLeave")
            }), at.on("lobbyPlayerGuessedWord", function(t) {
                var e = it.players.get(t);
                it.setPlayerGuessed(e), it.chatAddMsg(null, e.name + " guessed the word!", _), t == it.myID && it.chatDisable(), ct.playSound("playerGuessed")
            }), at.on("lobbyGuessClose", function(t) {
                it.chatAddMsg(null, "'" + t + "' is close!", N)
            }), at.on("lobbyPlayerDrawing", function(t) {
                var e = it.players.get(t);
                it.setPlayerDrawing(e.id), it.chatAddMsg(null, e.name + " is drawing now!", O);
                var n = it.drawingID == it.myID;
                ht.setDrawing(n), n ? (f(), y()) : (p(), d()), ct.playSound("roundStart"), D(it.timeMax), U()
            }), at.on("lobbyOwnerChanged", function(t) {
                var e = it.players.get(t);
                it.setPlayerOwner(e), it.chatAddMsg(null, e.name + " is the owner now!", _)
            }), at.on("lobbyRound", function(t) {
                it.round = t, it.updateRound(), L({
                    mode: "text",
                    text: "Round " + (it.round + 1)
                })
            }), at.on("lobbyChooseWord", function(t) {
                if (t.id == it.myID) L({
                    mode: "choosewords",
                    text: "Choose a word",
                    words: t.words
                    ...
                    L({
                        mode: "text",
                        text: it.players.get(t.id).name + " is choosing a word!"
                    })
                }
            }), at.on("lobbyReveal", function(t) {
                A(), it.chatEnable(), it.chatAddMsg(null, "The word was '" + t.word + "'", _);
                for (var e = [], n = !0, o = 0; o < t.scores.length / 2; o++) {
                    ...
                    if (i) {
                        i.guessedWord && (n = !1);
                        var a = s - i.score;
                        it.setPlayerScore(i, s, a), e.push(i)
                    }
                }
                it.updateRanks(), L({
                    mode: "reveal",
                    ...
                    players: e,
                    onshow: function() {
                        n ? ct.playSound("roundEndFailure") : ct.playSound("roundEndSuccess")
                    }
                }), it.setPlayerDrawing(-1), ht.setDrawing(!1), it.playersResetGuessedWord()
            }), at.on("lobbyPlayerRateDrawing", function(t) {
                var e = it.players.get(t[0]),
                    n = it.players.get(t[1]),
                    o = t[2];
                e && it.playerRate(e, o), n && (o > 0 ? n.thumbsUp++ : n.thumbsDown++)
            }), at.on("lobbyCurrentWord", function(t) {
                it.players.get(it.myID).guessedWord || E(t)
            }), at.on("lobbyTime", function(t) {
                S(t)
            })
        })
    }
    ...
    function A() {
        null != mt && (clearInterval(mt), mt = null)
    }

    function D(t) {
        S(t), A(), mt = setInterval(function() {
            gt > 0 && S(gt - 1)
        }, 1e3)
    }

    function S(t) {
        gt = t, gt <= 8 && ct.playSound("tick"), n("#timer").text(gt)
    }

    function P() {
        ut.goto("load"), at.emit("lobbyLeave")
    }
    ...
    function T(t, e, n) {
        if (it.checkDrawing() && (ht.updateMousePosition(t, e, n), ht.brush.down)) {
            var o = null;
            switch (ht.brush.tool) {
                case "pen":
                    o = ht.createDrawCommandLine(ht.brush.colorIndex, ht.brush.thickness, ht.mouseposPrev.x, ht.mouseposPrev.y, ht.mousepos.x, ht.mousepos.y);
                    break;
                case "erase":
                    o = ht.createDrawCommandErase(ht.brush.thickness, ht.mouseposPrev.x, ht.mouseposPrev.y, ht.mousepos.x, ht.mousepos.y);
                    break;
                case "fill":
                    ht.brush.toolUsed || (o = ht.createDrawCommandFill(ht.brush.colorIndex, ht.mouseposPrev.x, ht.mouseposPrev.y, ht.mousepos.x, ht.mousepos.y))
            }
            null != o && (ht.brush.toolUsed = !0, I(o))
        }
    }
    ...
    function I(t) {
        it.drawingID == it.myID ? (ht.drawCommands.push(t), ht.performDrawCommand(t)) : ht.addDrawCommandReceived(t)
    }

    function L(t) {
        if (pt) return lt.unshift(t), void U();
        pt = !0;
        var e = n("#overlay");
        e.show(), e.animate({
            opacity: "1"
    ...
    function R(t) {
        ...
        if ("choosewords" == t.mode) {
            ...
            for (var i = 0; i < t.words.length; i++) {
                ...
                a.data("id", i), a.text(t.words[i]), r.append(a), a.on("click", function() {
                    ...
                    at && at.emit("lobbyChooseWord", t), U()
                })
            }
        } else r.hide();
        ...
        if ("endgame" == t.mode) {
            ...
            for (var i = 0; i < t.players.length; i++) {
                ...
                var g = l.find(".rank");
                g.text("#" + h.rank), h.rank <= 3 && g.addClass("rank-" + h.rank), h.rank <= 3 ? y.append(l) : m.append(l), s(l, h.avatar, h.id == it.ownerID, h.rank <= 3 ? 2 : 1)
            }
        } else d.hide();
        o.show(), o.animate({
            bottom: "0%"
    ...
    const _ = "#56CE27",
        ...
        $ = 4;
    var K;
    K = void 0 !== t.localStorage ? t.localStorage : {
    ...
    var V = function(t, e) {
        ...
        Z = function() {
            this.context = null, this.sounds = new Map, this.mute = !1, "true" === K.getItem("audioMute") && this.setMute(!0), t.addEventListener("load", this.load.bind(this), !1)
        };
    Z.prototype.setMute = function(t) {
        this.mute = t, this.mute ? n("#audio").css("background-image", "url(res/audio_off.gif") : n("#audio").css("background-image", "url(res/audio.gif"), K.setItem("audioMute", this.mute.toString())
    }, Z.prototype.addSound = function(t, e) {
        this.sounds.set(e, new V(this, t))
    }, Z.prototype.loadSounds = function() {
    ...
    }, Z.prototype.playSound = function(t) {
    ...
    var Q = function(t) {
        this.canvas = t, this.canvasCtx = this.canvas[0].getContext("2d"), this.canvasCtx.mozImageSmoothingEnabled = !1, this.canvasCtx.webkitImageSmoothingEnabled = !1, this.canvasCtx.msImageSmoothingEnabled = !1, this.canvasCtx.imageSmoothingEnabled = !1, this.mouseposPrev = {
            ...
            x: 0,
            y: 0
        }, this.drawCommands = [], this.drawCommandsReceived = [], this.brush = new tt;
        var e = this;
        setInterval(function() {
            e.drawCommandsReceived.length > 0 && e.performDrawCommand(e.drawCommandsReceived.shift())
        }, 3), this.clear()
    };
    Q.prototype.updateMousePosition = function(t, e, n) {
    ...
    }, Q.prototype.addDrawCommandReceived = function(t) {
        this.drawCommandsReceived.push(t)
    }, Q.prototype.addDrawCommand = function(t) {
        this.drawCommands.push(t)
    }, Q.prototype.createDrawCommandLine = function(t, e, n, o, r, s) {
        return [0, t, e, n, o, r, s]
    }, Q.prototype.createDrawCommandErase = function(t, e, n, o, r) {
        return [1, t, e, n, o, r]
    }, Q.prototype.createDrawCommandFill = function(t, e, n) {
        return [2, t, e, n]
    }, Q.prototype.performDrawCommand = function(t) {
        switch (t[0]) {
            case 0:
    ...
    }, Q.prototype.plotLineOld = function(t, e, n, o, r, s) {
    ...
    }, Q.prototype.plotLine = function(t, e, n, o, r, s, i, a) {
    ...
    }, Q.prototype.setDrawing = function(t) {
    ...
    }, tt.prototype.updateBrushCursor = function() {
        switch (this.tool) {
            case "pen":
            case "erase":
    ...
    et.prototype.updatePlayerAvatars = function() {
        var t = this;
    ...
    }, et.prototype.containerGamePlayerClear = function() {
        this.containerGame.empty()
    }, et.prototype.containerGamePlayerGet = function(t) {
        return this.containerGame.find("#player" + t.id)
    }, et.prototype.containerGamePlayerAdd = function(t) {
    ...
    }, et.prototype.containerLobbyPlayerRemove = function(t) {
        this.containerLobbyPlayerGet(t).remove()
    }, et.prototype.containerLobbyPlayersUpdateOwner = function() {
        var t = this;
        this.players.forEach(function(e, n, o) {
    ...
    }, et.prototype.containerLobbyPlayerClear = function() {
        this.containerLobby.empty()
    }, et.prototype.containerLobbyUpdateRounds = function() {
        n("#lobbySetRounds").val(this.roundMax)
    }, et.prototype.containerLobbyUpdateDrawTime = function() {
        n("#lobbySetDrawTime").val(this.timeMax)
    }, et.prototype.containerLobbyUpdateUseCustomWordsExclusive = function() {
        n("#lobbyCustomWordsExclusive").attr("checked", !!this.useCustomWordsExclusive)
    }, et.prototype.containerLobbyUpdateLanguage = function() {
        var t = M(this.language);
        n("#lobbySetLanguage").val(t)
    }, et.prototype.playerMessage = function(t, e) {
        this.playerMessageHandle(t);
        var n = this.containerGamePlayerGet(t).find(".message");
        n.find(".text").show(), n.find(".text").text(e)
    }, et.prototype.playerRate = function(t, e) {
        this.playerMessageHandle(t);
        var n = this.containerGamePlayerGet(t).find(".message");
        e > 0 ? n.find(".iconThumbsUp").show() : n.find(".iconThumbsDown").show()
    }, et.prototype.playerMessageHandle = function(t) {
    ...
    }, et.prototype.chatClear = function() {
        n("#boxMessages").empty()
    }, et.prototype.chatAddMsg = function(t, e, o) {
        var r = n("#boxMessages"),
        ...
        if (a.css("color", o), a.appendTo(r), t) {
            n("<b/>", {
                text: t.name + ": "
            }).appendTo(a), this.playerMessage(t, e)
        } else a.css("font-weight", "bold");
        n("<span/>", {
            text: e
        }).appendTo(a), i && r.scrollTop(r[0].scrollHeight)
    }, et.prototype.chatDisable = function() {}, et.prototype.chatEnable = function() {};
    var nt = function(t, e) {
    ...
    var ot = function() {
        this.screens = {
            load: new nt(n("#screenLoading"), 0),
            login: new nt(n("#screenLogin"), 2),
            lobby: new nt(n("#screenLobby"), 1),
            game: new nt(n("#screenGame"), 1)
        }, this.screenCurrent = null
    };
    ot.prototype.goto = function(t) {
    ...
    }, ot.prototype.hideLogo = function() {
        n("#containerLogoBig").hide(), n("#containerLogoSmall").hide()
    }, ot.prototype.showLogo = function(t) {
    ...
    var rt = t.location != t.parent.location ? e.referrer : e.location.href,
        st = function(t, e) {
            return -1 !== t.indexOf(e)
        };
    (st(rt, "skribbl-io.com") || st(rt, "skribblio.com") || st(rt, "skribbl-io.org") || st(rt, "skribblio.online")) && (t.top.location.href = "https://www.skribbl.io"), adplayer = null, aiptag.cmd.player.push(function() {
    ...
    var it = null,
        ...
        ct = new Z,
        ut = new ot,
        ht = new Q(n("#canvasGame")),
        lt = new Array,
        pt = !1,
        ft = !1;
    ht.setDrawing(!1), ut.goto("login");
    var dt = !1;
    ut.screens.game.onshow = function() {
            dt || (dt = !0, (adsbygoogle = t.adsbygoogle || []).push({}))
        ...
        function() {
            ...
            K.setItem("name", n("#inputName").val())
        }), null != K.getItem("name") && n("#inputName").val(K.getItem("name")), n("#loginLanguage").change(function() {
            K.setItem("lang", n("#loginLanguage").val())
        }), null != K.getItem("lang") && n("#loginLanguage").val(K.getItem("lang")), loginAvatarData = function() {
            var t = K.getItem("avatar");
            return t ? t = JSON.parse(t) : (t = [Math.round(Math.random() * J[0]), Math.round(Math.random() * J[1]), Math.round(Math.random() * J[2]), -1], K.setItem("avatar", JSON.stringify(t))), t
    ...
    S(80), n("#lobbySetLanguage").on("change", function() {
        ...
        ut.goto("load"), at.emit("joinRandom")
    }), n("#buttonLobbyLeave").on("click", P), n("#buttonGameLeave").on("click", P), n("#buttonLobbyPlay").on("click", function() {
        at && it.myID == it.ownerID && at.emit("lobbyGameStart", n("#lobbySetCustomWords").val())
    }), n("#formChat").submit(function(t) {
        var e = n("#inputChat"),
            o = e.val();
        return o && (at ? at.emit("chat", o) : it.chatAddMsg(it.players.get(it.myID), o, "#000"), e.val("")), !1
        ...
        at.emit("lobbyCreate", t), n("#modalCreateLobby").modal("hide")
    }), n("#buttonRefreshLobbies").on("click", function() {
        at.emit("requestLobbies")
    }), ht.canvas.on("mousewheel DOMMouseScroll", function(t) {
        t.preventDefault();
        var e = t.originalEvent.wheelDelta > 0 || t.originalEvent.detail < 0 ? 1 : -1;
        ht.brush.setThickness(ht.brush.thickness + 6 * e)
    }), n(e).on("mousemove", function(t) {
        it && it.checkDrawing() && T(t.clientX, t.clientY, !1)
    }), n(e).keypress(function(t) {
        if (it && it.checkDrawing() && t !== r && t.key !== r) switch (t.key.toUpperCase()) {
            case "B":
                ht.brush.setTool("pen");
                break;
            case "E":
                ht.brush.setTool("erase");
                break;
            case "F":
                ht.brush.setTool("fill")
        }
    }), ht.canvas.on("mousedown", function(t) {
        switch (t.preventDefault(), t.button) {
            case 0:
                ht.brush.down || (ht.brush.setDown(!0), T(t.clientX, t.clientY, !0))
        }
    });
    var vt = null;
    ht.canvas.on("touchstart", function(t) {
        t.preventDefault();
        var e = t.changedTouches;
        e.length > 0 && null == vt && (vt = e[0].identitfier, ht.brush.setDown(!0), T(e[0].clientX, e[0].clientY, !0))
    }), ht.canvas.on("touchend", function(t) {
        t.preventDefault(), vt = null, ht.brush.setDown(!1)
    }), ht.canvas.on("touchcancel", function(t) {
        t.preventDefault(), vt = null, ht.brush.setDown(!1)
    }), ht.canvas.on("touchmove", function(t) {
        t.preventDefault();
        for (var e = t.changedTouches, n = 0; n < e.length; n++)
            if (e[n].identitfier == vt) {
        ...
        at ? at.emit("canvasClear") : ht.clear()
    }), n(".containerTools .tool").on("click", function() {
        ht.brush.setTool(n(this).data("tool"))
    }), n(".colorItem").on("click", function() {
        var t = n(this).data("color");
        ht.brush.setColor(Number(t))
    }), n(".brushSize").on("click", function() {
        var t = ht.brush.thicknessMin,
            e = ht.brush.thicknessMax,
            o = Number(n(this).data("size")) * (e - t) + t;
        ht.brush.setThickness(o)
    })
